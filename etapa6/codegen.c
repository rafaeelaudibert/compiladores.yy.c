...
TAC *generate_func_param(TAC *expression, TAC *other_param)
{
    ...
    TAC *val = tac_create(TAC_FUNC_PARAM, NULL, RES_OR_NULL(expression), NULL),
        *expr_and_val = tac_join(expression, val);

    return tac_join(expr_and_val, other_param);
}
...
TAC *generate_code(AST *node)
{
    ...
    case AST_FUNC_CALL:
        ...
        break;
    case AST_LPARAMETER:
        result = generate_func_param(children[0], children[1]);
        break;
