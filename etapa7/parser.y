%{
    ...
    void* THROW_ERROR(char* error_str) {
        syntax_error = 1;
        fprintf(stdout, "Syntax error at line %d: %s\n", getLineNumber(), error_str);
...
ldecl: declv ';' ldecl  { $$ = AST_create( AST_LDECL, NULL, $1, $3, NULL, NULL, NULL); }
    ...
    | declv error ldecl { $$ = THROW_ERROR("Missing ;"); }
    | declf error ldecl { $$ = THROW_ERROR("Missing ;"); }
    |                   { $$ = NULL; }
    ;
...
declv_not_vector: TK_IDENTIFIER '=' any_kw_type ':' any_lit     { $$ = AST_create( AST_DECLV_NOT_VECTOR, NULL, SYMBOL_AST($1), $3, $5, NULL, NULL); }
    | TK_IDENTIFIER '=' any_kw_type error                       { $$ = THROW_ERROR("Missing initialization value for variable"); }
    ;
...
declv_vector: TK_IDENTIFIER '=' any_kw_type '[' LIT_INTEGER ']' ':' lvector { $$ = AST_create( AST_DECLV_VECTOR, NULL, SYMBOL_AST($1), $3, SYMBOL_AST($5), $8, NULL); }
    ...
    | TK_IDENTIFIER '=' any_kw_type '[' error ']'  ':' lvector              { $$ = THROW_ERROR("Vector size must be an integer"); }
    | TK_IDENTIFIER '=' any_kw_type '[' error ']'                           { $$ = THROW_ERROR("Vector size must be an integer"); }
    ;
...
function_header: TK_IDENTIFIER function_params '=' any_kw_type  { $$ = AST_create( AST_FUNC_HEADER, NULL, SYMBOL_AST($1), $2, $4, NULL, NULL); }
    | TK_IDENTIFIER error '=' any_kw_type                       { $$ = THROW_ERROR("Expected parameters declaration between parenthesis"); }
    ;
...
lparam: TK_IDENTIFIER '=' any_kw_type rest_params   { $$ = AST_create( AST_FUNC_PARAMS, NULL, SYMBOL_AST($1), $3, $4, NULL, NULL); }
    | TK_IDENTIFIER error rest_params           { $$ = THROW_ERROR("Expected a type keyword declaration"); }
    ;
...
lcommand: command lcommand                          { $$ = AST_create( AST_LCOMMAND, NULL, $1, $2, NULL, NULL, NULL); }
    | error lcommand                                { $$ = THROW_ERROR("Expected a valid command"); }
...
flow_command: KW_IF '(' expression ')' KW_THEN command                                      { $$ = AST_create(AST_IF, NULL, $3, $6, NULL, NULL, NULL); }
    ...
    | KW_IF error KW_THEN command           { $$ = THROW_ERROR("Expected an if condition between parenthesis"); }
    | KW_IF error KW_THEN command KW_ELSE command           { $$ = THROW_ERROR("Expected an if condition between parenthesis"); }
    | KW_WHILE error command                { $$ = THROW_ERROR("Expected an while condition between parenthesis"); }
    | KW_LOOP error command                 { $$ = THROW_ERROR("Expected a loop condition between parenthesis"); }
    ;
...
expression: TK_IDENTIFIER                       { $$ = SYMBOL_AST($1); }
    ...
    | TK_IDENTIFIER '[' error ']'               { $$ = THROW_ERROR("Expected a valid expression for indexing the vector"); }
    | any_lit                                   { $$ = $1; }
    | '(' expression ')'                        { $$ = $2; }
...
rest_params_func_call: ',' lparameter_func_call                     { $$ = $2; }
    | error lparameter_func_call                                    { $$ = THROW_ERROR("Expected comma between function call parameters"); }
    |                                                               { $$ = NULL; }
    ;
